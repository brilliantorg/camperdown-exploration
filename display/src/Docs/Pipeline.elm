...
and `mydoc` is a string representing a Camperdown document, it suffices
to say `parse standard mydoc` to obtain the AST of `mydoc`.  The type of the AST is
...
In step (1), the source text is broken up into parts.  The _prelude_
is the text appearing before the first section marker (`#`, `##` or `###`),
while the sections are derived from the text between markers.  We say "derived", because the type `Section` is slightly more elaborate than a list
of lines.  It contains information on the location of the section in
the source text, indentation, and the "label", which is either the
...
`Named "Intro"`, while if no text follows the section marker, the label is
`Anonymous k` for some integer k.
